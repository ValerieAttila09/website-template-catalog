...
export default function PreviewShell({ title, slug, previewUrl, templateId, category, techStack, salesCount }: PreviewShellProps) {
  ...
  return (
    <main className="flex h-screen w-screen flex-col overflow-hidden bg-[#505F62] text-[#F9F2E7]">
      <header className="z-20 shrink-0 border-b border-[#D0BDA8]/30 bg-[#505F62] px-4 py-3 sm:px-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-4">
            ...
            <span className="hidden text-sm font-bold text-[#FFCF00] md:block">{licensePrices[license]}</span>
            <a href={checkoutUrl} className="rounded-xl bg-[#F18744] px-3 py-2 text-xs font-extrabold text-[#F9F2E7] transition hover:bg-[#FFCF00] hover:text-[#505F62] sm:px-4 sm:py-2.5">Buy template <span aria-hidden="true">↗</span></a>
          </div>
        </div>
      </header>
      ...
      {isQrOpen && (
        <div className="fixed inset-0 z-40" role="presentation">
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="qr-preview-title" className="absolute right-4 top-[4.75rem] w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-[#D0BDA8]/30 bg-[#505F62] p-5 text-center shadow-2xl shadow-black/50 sm:right-6">
            ...
            <div className="mx-auto flex size-[196px] items-center justify-center rounded-xl bg-white p-2">
              {shareUrl && <QRCodeCanvas value={shareUrl} size={180} bgColor="#ffffff" fgColor="#505F62" level="M" includeMargin={false} />}
            </div>
      ...
      <section className="relative flex min-h-0 h-[calc(100vh-45px)] flex-1 items-start justify-center overflow-hidden bg-[#1c2523]">
        ...
        <aside className={`relative flex h-full shrink-0 flex-col border-l border-[#D0BDA8]/30 bg-[#505F62] transition-all duration-300 ease-in-out ${isSidebarOpen ? 'w-80' : 'w-12'}`} aria-label="Preview controls and template information">
          ...
          {isSidebarOpen ? <>
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-4">
              <div><p className="flex items-center gap-2 text-xs font-extrabold text-[#F9F2E7]"><Cpu className="size-3.5 text-[#FFCF00]" /> Preview controls</p><p className="mt-1 text-[10px] text-[#F9F2E7]/55">Shape the viewport and explore details</p></div>
            ...
            <div className="preview-scrollbar flex-1 space-y-5 overflow-y-auto p-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb:hover]:bg-lime-300/40 [&::-webkit-scrollbar-track]:bg-transparent">
              <section>
                ...
                <div className="mt-2 flex items-center gap-2">
                  {mode !== 'desktop' && <button type="button" onClick={() => setOrientation((current) => current === 'portrait' ? 'landscape' : 'portrait')} aria-pressed={orientation === 'landscape'} className={`inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border px-2 py-2 text-[10px] font-bold transition ${orientation === 'landscape' ? 'border-[#FFCF00]/40 bg-[#FFCF00] text-[#505F62]' : 'border-white/10 bg-white/5 text-slate-300 hover:bg-white/10'}`}><RotateCw className="size-3.5" />{orientation === 'portrait' ? 'Landscape' : 'Portrait'}</button>}
              ...
              <section>
                <div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#F9F2E7]/55">Choose your license</p><span className="text-[10px] font-bold text-[#FFCF00]">{licensePrices[license]}</span></div>
                <div className="space-y-2">
                  {(['personal', 'commercial'] as License[]).map((item) => <label key={item} className={`flex cursor-pointer items-center justify-between rounded-xl border p-3 transition ${license === item ? 'border-[#FFCF00]/50 bg-[#FFCF00]/10' : 'border-white/10 bg-white/[0.03] hover:border-white/20'}`}><span className="flex items-center gap-2"><input type="radio" name="license" value={item} checked={license === item} onChange={() => setLicense(item)} className="accent-[#FFCF00]" /><span><span className="block text-xs font-bold text-white">{item === 'personal' ? 'Personal License' : 'Commercial License'}</span><span className="mt-0.5 block text-[10px] text-slate-500">{item === 'personal' ? 'For one personal project' : 'For client and business work'}</span></span></span><span className="text-xs font-extrabold text-[#FFCF00]">{licensePrices[item]}</span></label>)}
                </div>
              </section>
              ...
              <section className="rounded-2xl border border-[#D0BDA8]/25 bg-white/[0.03] p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">{(techStack.length > 0 ? techStack : ['Next.js', 'React', 'Tailwind CSS']).map((technology) => <span key={technology} className="rounded-md border border-[#FFCF00]/30 bg-[#FFCF00]/10 px-2 py-1 text-[10px] font-bold text-[#FFCF00]">{technology}</span>)}</div>
              </section>
            ...
            <div className="space-y-2 border-t border-white/10 p-4">
              <a href={checkoutUrl} className="block rounded-xl bg-[#F18744] px-4 py-3 text-center text-xs font-extrabold text-[#F9F2E7] transition hover:bg-[#FFCF00] hover:text-[#505F62]">Buy Template · {licensePrices[license]}</a>
